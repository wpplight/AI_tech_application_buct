...
<style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .header { background: linear-gradient(135deg, #042f2e 0%, #064e3b 50%, #065f46 100%); padding: 0; position: sticky; top: 0; z-index: 100; box-shadow: 0 4px 30px rgba(0,0,0,0.3); }
    ...
    .algo-badge.incremental { background: linear-gradient(135deg, #0d9488, #14b8a6); box-shadow: 0 2px 10px rgba(13,148,136,0.4); }
    ...
    .rule-item { padding: 10px 12px; margin-bottom: 6px; background: rgba(13,148,136,0.08); border-left: 3px solid #0d9488; border-radius: 8px; font-size: 13px; cursor: pointer; transition: all 0.2s; color: #cbd5e1; }
    .rule-item:hover { background: rgba(13,148,136,0.15); transform: translateX(2px); }
    ...
    @keyframes rule-flash { 0%,100% { background: rgba(13,148,136,0.08); } 20% { background: rgba(250,204,21,0.15); box-shadow: 0 0 15px rgba(250,204,21,0.2); } 50% { background: rgba(250,204,21,0.2); box-shadow: 0 0 25px rgba(250,204,21,0.3); } 80% { background: rgba(250,204,21,0.15); } }
    .rule-highlight { animation: rule-flash 1.2s ease-in-out 2; }
    .rule-link { color: #14b8a6; cursor: pointer; text-decoration: underline; font-weight: 600; }
    .rule-link:hover { color: #2dd4bf; }
    ...
    .btn-primary:hover { background: linear-gradient(135deg, #14b8a6, #0d9488); }
    ...
    .btn-outline { background: transparent; color: #14b8a6; border: 1px solid rgba(20,184,166,0.3); }
    .btn-outline:hover { background: rgba(13,148,136,0.1); border-color: rgba(20,184,166,0.5); }
    ...
    .input-group input:focus { border-color: #0d9488; box-shadow: 0 0 0 3px rgba(13,148,136,0.15); }
    ...
    .badge-blue { background: rgba(13,148,136,0.15); color: #14b8a6; }
    .badge-green { background: rgba(34,197,94,0.15); color: #4ade80; }
    .badge-orange { background: rgba(249,115,22,0.15); color: #fb923c; }
    .section { margin-bottom: 20px; }
    .section-title { font-size: 12px; font-weight: 600; margin-bottom: 10px; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px; }
    ...
    .goal-input input:focus { border-color: #0d9488; }
    ...
    .tab.active { background: rgba(13,148,136,0.2); color: #14b8a6; }
...
<div class="header">
    <div class="header-top">
        <div class="header-left">
            <h1>🦁 动物识别专家系统</h1>
            <span class="algo-badge incremental">⚡ Incremental</span>
        </div>
        <div class="header-right">
            <span class="port-tag">PORT 8081</span>
        </div>
    </div>
    <div class="header-nav">
        <a href="http://localhost:8080">全扫描</a>
        <a href="http://localhost:8081" class="active">增量触发</a>
...
<div class="container">
    ...
    <div class="panel center">
        ...
        <div class="section">
            ...
            <p style="font-size:12px;color:#94a3b8;margin-bottom:6px;">① 输入已知事实，② 点击执行推理</p>
            ...
            <div style="font-size:12px;color:#94a3b8;margin-bottom:8px;">
                ...
                <a href="#" onclick="switchTabByName('facts');return false;" style="color:#14b8a6;">查看详情</a>
            </div>
        ...
        <div class="section">
            ...
            <p style="font-size:12px;color:#94a3b8;margin-bottom:8px;">验证特定目标是否成立</p>
...
<div class="modal-overlay" id="deleteRuleModal">
    <div class="modal">
        <h3>删除规则</h3>
        <p style="font-size:13px;color:#94a3b8;margin-bottom:8px;">选择要删除的规则：</p>
        <input type="text" id="deleteRuleSearch" placeholder="搜索规则（输入条件或结论关键词）..." oninput="filterDeleteRuleList()" style="width:100%;padding:8px;border:1px solid rgba(255,255,255,0.1);border-radius:6px;font-size:13px;margin-bottom:8px;outline:none;background:rgba(15,23,42,0.6);color:#e2e8f0;">
...
<div class="modal-overlay" id="modifyRuleModal">
    <div class="modal">
        ...
        <div class="input-group">
            ...
            <input type="text" id="modifyRuleSearch" placeholder="输入关键词搜索..." oninput="filterModifyRuleList()" style="width:100%;padding:8px;border:1px solid rgba(255,255,255,0.1);border-radius:6px;font-size:13px;margin-bottom:6px;outline:none;background:rgba(15,23,42,0.6);color:#e2e8f0;">
            <div class="scroll-area" id="modifyRuleList" style="max-height:150px;border:1px solid rgba(255,255,255,0.08);border-radius:6px;padding:4px;"></div>
        </div>
        <div class="input-group">
            <label>条件（用逗号分隔）</label>
...
<div class="modal-overlay" id="deleteFactModal">
    <div class="modal">
        <h3>删除事实</h3>
        <p style="font-size:13px;color:#94a3b8;margin-bottom:12px;">选择要删除的事实：</p>
...
async function runBackward() {
    ...
    if (data.missing_facts && data.missing_facts.length > 0) {
        ...
        data.missing_facts.forEach(f => {
            ...
            btn.style.cssText = 'padding:4px 10px;font-size:12px;background:rgba(13,148,136,0.15);color:#14b8a6;border:1px solid rgba(20,184,166,0.3);border-radius:4px;cursor:pointer;';
            btn.textContent = '+ ' + f;
            btn.onclick = async () => {
...
async function loadSteps() {
    ...
    el.innerHTML = data.steps.map((s, i) => {
        let html = '', bgColor = 'rgba(249,115,22,0.08)', borderColor = '#f97316';
        if (s.type === 'forward') {
            ...
            bgColor = 'rgba(34,197,94,0.08)'; borderColor = '#22c55e';
        } else if (s.type === 'backward') {
            if (s.result === '已知事实') {
                html = `<strong>步骤 ${i+1}</strong>: 反向推理 → 目标"${s.goal}" <span style="color:#4ade80;">✓ 已在事实库</span>`;
                bgColor = 'rgba(34,197,94,0.08)'; borderColor = '#22c55e';
            } else if (s.result === '无规则可推导') {
                html = `<strong>步骤 ${i+1}</strong>: 反向推理 → 目标"${s.goal}" <span style="color:#f87171;">✗ 无匹配规则</span>`;
                bgColor = 'rgba(239,68,68,0.08)'; borderColor = '#ef4444';
            } else if (s.attempt) {
                const ruleStr = `IF ${s.rule_conditions.join(' AND ')} THEN ${s.rule_conclusion}`;
                html = `<div style="display:flex;justify-content:space-between;align-items:flex-start;">
                ...
                <div style="margin-top:4px;font-size:12px;">规则 ${s.rule_id}: ${ruleStr}</div>
                <div style="margin-top:2px;font-size:12px;color:#64748b;">需要条件: ${s.conditions.join(', ')}</div>`;
                bgColor = 'rgba(249,115,22,0.08)'; borderColor = '#f97316';
                ...
                const ruleStr = `IF ${s.rule_conditions.join(' AND ')} THEN ${s.rule_conclusion}`;
                html = `<div style="display:flex;justify-content:space-between;align-items:flex-start;">
                    <div><strong>步骤 ${i+1}</strong>: 反向推理 → 推导"${s.goal}" <span style="color:#f87171;">✗ 条件不满足</span></div>
                    <span class="rule-link" onclick="highlightRule(${s.rule_id})">跳转 Rule ${s.rule_id}</span>
                </div>
                <div style="margin-top:4px;font-size:12px;">规则 ${s.rule_id}: ${ruleStr}</div>`;
                bgColor = 'rgba(239,68,68,0.08)'; borderColor = '#ef4444';
            } else if (s.result === '循环依赖，跳过') {
                html = `<strong>步骤 ${i+1}</strong>: 反向推理 → 目标"${s.goal}" <span style="color:#fbbf24;">⚠ 循环依赖，跳过</span>`;
                bgColor = 'rgba(245,158,11,0.08)'; borderColor = '#f59e0b';
            } else {
...
function selectDeleteRule(el, id) {
    document.querySelectorAll('#deleteRuleList .rule-item').forEach(e => e.style.background = 'rgba(13,148,136,0.08)');
...
function selectModifyRule(el, id) {
    document.querySelectorAll('#modifyRuleList .rule-item').forEach(e => {
        e.style.background = 'rgba(13,148,136,0.08)';
        e.style.borderLeftColor = '#0d9488';
    });
