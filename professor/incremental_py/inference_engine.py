...
class InferenceEngine:
    ...
    def forward_chain(self, max_iterations=100):
        ...
        known_facts = set(self.fb.get_facts())
        triggered = self._collect_triggered_rules(known_facts)
        checked = set()

        while triggered and iteration < max_iterations:
            newly_deduced = []
            for rule in list(triggered):
            ...
            triggered = self._collect_triggered_rules(set(newly_deduced))
            triggered = [r for r in triggered if r.id not in checked]
            iteration += 1

        return new_facts

    def _collect_triggered_rules(self, facts):
        rule_ids = set()
        for fact in facts:
            rule_ids.update(
                r.id for r in self.kb.get_rules_triggered_by(fact)
            )
        if not rule_ids:
            return []
        return [self.kb._rule_from_db(rid) for rid in rule_ids]
