...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let y_tensor = x_tensor.map(|_indices, x| {
        formula::normal_distribution(x, 0.0, 1.0)
    });
    ...
    let n = x_tensor.data().len();
    let mut points = Vec::with_capacity(n * 2);
    for i in 0..n {
        points.push(x_tensor.data()[i]);
        points.push(y_tensor.data()[i]);
    }
    let points_tensor = Tensor::build(points, vec![n, 2])?;
    ...
    let config = PlotConfig::new()
        ...
        .y_ticks(6)
        .show_window(true)
        .export("output/normal_distribution.svg");

    draw::plot(&config, &[&points_tensor], &["f(x) = N(0,1)"])?;
