use nn::{Linear, Loss, Module, Sequential, Tanh};
use tensor::Tensor;

pub fn run() -> Result<(), Box<dyn std::error::Error>> {
    println!("╔════════════════════════════════════════════════════════╗");
    println!("║                    任务2: 正弦回归 y = sin(x)            ║");
    println!("╚════════════════════════════════════════════════════════╝\n");
    ...
    let (x_train, y_train) = generate_sin_data(&mut rng, num_train, 0.0, 6.28);
    let (x_val, y_val) = generate_sin_data(&mut rng, num_val, 0.0, 6.28);
    let (x_test, y_test) = generate_sin_data(&mut rng, num_test, 6.28, 12.56);
    ...
    draw::plot(
        &draw::PlotConfig::new()
            ...
            .ylabel("Loss")
            .show_window(false)
            .export("output/regression/sin/训练.svg"),
    ...
    draw::plot(
        &draw::PlotConfig::new()
            ...
            .dot(true)
            .show_window(false)
            .export("output/regression/sin/猜想.svg"),
    ...
    draw::plot(
        &draw::PlotConfig::new()
            ...
            .dot(true)
            .show_window(false)
            .export("output/regression/sin/泛化.svg"),
        &[&create_scatter_points(&mut model, &x_test, &y_test)?],
        &["测试集预测"],
    )?;
    
    draw_fit_curve(&mut model, 0.0, 6.28, 100)?;
    
    println!("  ✅ 已保存: output/regression/sin/训练.svg");
    println!("  ✅ 已保存: output/regression/sin/猜想.svg");
    println!("  ✅ 已保存: output/regression/sin/泛化.svg");
    println!("  ✅ 已保存: output/regression/sin/fit_comparison.svg");
...
fn draw_fit_curve(
    ...
    draw::plot(
        &draw::PlotConfig::new()
            .title("正弦回归 y = sin(x) (对比)")
            .xlabel("x")
            .ylabel("y")
            .show_window(false)
            .export("output/regression/sin/fit_comparison.svg"),
